# LE Battery Service Client Sample application
...
3. On application start, push the User button on the board and release after the message "Release now to start pairing." is displayed in the traces, this causes the Battery Client App to scan and connect to the Battery Service Server, which would have UUID\_SERVICE\_BATTERY in its advertisements.
4. If no Battery Service Server device is found nearby for 90 seconds, then the scan stops automatically. To restart the scan, push the button on the board and release after the message "Release now to start pairing." is displayed.
