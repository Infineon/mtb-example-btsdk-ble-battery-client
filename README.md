# BLE Battery Service Client Sample application

## Overview
The Battery Service Client application is designed to connect to and access services of the Battery Service Server application. The Battery Client connects to a device advertising the Battery Service UUID.
...
3. On application start, push the User button on the board and release within 2 seconds, this causes the Battery Client App to scan and connect to the Battery Service Server, which would have UUID\_SERVICE\_BATTERY in its advertisements.
4. If no Battery Service Server device is found nearby for 90 seconds, then the scan stops automatically. To restart the scan, push the button on the board and release within 2 secs.
5. Upon successful Connection, the Battery Client App will discover all the characteristics/descriptors of the server device.
6. Once the connection is established with the BLE peripheral (Battery Service found in the Peripheral), the application can enable/disable notifications, to control the reception of notifications of changes in the battery level. To enable/disable notifications from the server, push the button on the board and release after 5 seconds.
7. To read the battery level of the server, push the button on the board and release between 2-4 seconds.
8. For 20721B2 based WICED kits, use the Custom button on audio shield board as application button rather than the button on the base board.
